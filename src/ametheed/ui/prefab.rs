...
// use crate::ametheed::assets::prefab::AssetPrefab;
// use crate::ametheed::ui::button::UiButton;
use crate::ametheed::ui::widgets::Widgets;
...
// use crate::ametheed::assets::system::PrefabLoaderSystem;
...
use crate::ametheed::{
//     assets::progress::{Progress, ProgressCounter},
    core::hidden::{HiddenPropagate},
    ui::{transform::UiTransform,
         event::Interactable,
         selection::Selectable,
         drag::Draggable,
         layout::{Anchor, Stretch},
         text::{UiText, LineMode, TextEditing},
         widgets::{WidgetId},
    },
    error::Error,
    // assets::prefab::{Prefab, PrefabData},
    // assets::storage::Handle,
    ui::button::UiButtonActionType,
};
use std::{
    fmt::{Debug, Formatter},
    marker::PhantomData,
...
pub struct UiTextData {
    ...
    /// Font color
    pub color: [f32; 4],
    // TODO let the browser handle this?
    // /// Font
    ...
    // TODO how to have the text-edit concept inside the ui
...
//         _: &[Entity],
//     ) -> Result<(), Error> {
//         let (ref mut texts, ref mut editables) = system_data;
//         // let font_handle = self
//         //     .font
//         //     .as_ref()
//         //     .ok_or_else(|| error!("did not load sub assets"))?
...
pub enum UiWidget<// C = NoCustomUi,
                  W = u32, G = ()>
where
    // C: ToNativeWidget<W>,
    W: WidgetId,
...
//             .with_context(|_| format!("Failed deserializing Ron file"))?;
//         let root: UiWidget<C, W> = UiWidget::deserialize(&mut d)
//             .with_context(|_| format!("Failed parsing Ron file"))?;
//         d.end()
//             .with_context(|_| format!("Failed parsing Ron file"))?;
