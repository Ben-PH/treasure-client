...
use crate::ametheed::{Interactable, UiTransform};
...
macro_rules! define_widget_component_fn_impl {
    ( (has $t:ty as $name:ident on $entity:ident) ) => {
        paste::item! {
            ...
            pub fn [<get_ $name>]<'a>(
                &self,
                storage: &'a specs::ReadStorage<'a, $t>
            ) -> &'a $t {
        ...
        paste::item! {
            ...
            pub fn [<get_ $name _mut>]<'a>(
                &self,
                storage: &'a mut specs::WriteStorage<'a, $t>
            ) -> &'a mut $t {
                // TODO: Better error message
...
macro_rules! define_widget {
    ($t:ident =>
        ...
        pub struct $t {
            $(
                /// `$field` Entity
                pub $field: $crate::ametheed::Entity
            ),*
        }

        impl $crate::ametheed::ui::widgets::Widget for $t {}

        impl $t {
            /// Create a new $t widget from its associated entities.
            pub fn new(
                $($field: $crate::ametheed::Entity),*
            ) -> Self {
                Self {
                    $($field),*
                }
